...
def eval_model(args):
    ...
    gpt_file = open(answers_file.replace('.json', '_gpt.json'), "w")
    ques_file = open(answers_file.replace('.json', '_ques.json'), "w")
    context_file = open(answers_file.replace('.json', '_context.json'), "w")
    ...
    for line in tqdm(questions):
        ...
        ans_file.write(json.dumps({"answer_id": line['question_id'], "text": outputs}) + "\n")
        ans_file.flush()
        gpt_file.write(json.dumps({"answer_id": line['question_id'], "text": line['gpt_answer']}) + "\n")
        gpt_file.flush()
        ques_file.write(json.dumps({"image": line['question_id'], "category": line['category'], "text": line['question'], "question_id": line['question_id']}) + "\n")
        ques_file.flush()
        context_file.write(json.dumps({"image": line['question_id'], "caption": line['caption']}) + "\n")
        context_file.flush()
        question_idx += 1
